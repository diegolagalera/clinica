...
async function seed() {
    ...
    try {
        ...
        await index_js_1.db.execute((0, drizzle_orm_1.sql) `CREATE EXTENSION IF NOT EXISTS unaccent`);
        ...
        const [superAdmin] = await index_js_1.db.insert(schema_js_1.users).values({
        ...
        const [organization] = await index_js_1.db.insert(schema_js_1.organizations).values({
        ...
        const [clinic] = await index_js_1.db.insert(schema_js_1.clinics).values({
        ...
        const [admin] = await index_js_1.db.insert(schema_js_1.users).values({
        ...
        const [worker] = await index_js_1.db.insert(schema_js_1.users).values({
        ...
        const [patientUser] = await index_js_1.db.insert(schema_js_1.users).values({
        ...
        await index_js_1.db.insert(schema_js_1.patients).values({
            clinicId: clinic.id,
