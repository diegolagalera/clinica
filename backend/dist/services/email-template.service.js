...
exports.getTemplateTypes = exports.replaceVariables = exports.renderBlocksToHtml = exports.getDefaultTemplate = exports.deleteTemplate = exports.updateTemplate = exports.createTemplate = exports.getActiveTemplate = exports.getTemplateById = exports.getTemplates = exports.TEMPLATE_VARIABLES = void 0;
 ...
 */
const getTemplates = async (clinicId) => {
    return index_js_1.db.query.emailTemplates.findMany({
        where: (0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.clinicId, clinicId),
 ...
 */
const getTemplateById = async (id, clinicId) => {
    return index_js_1.db.query.emailTemplates.findFirst({
        where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.id, id), (0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.clinicId, clinicId)),
    });
};
 ...
 */
const getActiveTemplate = async (clinicId, type) => {
    logger_js_1.logger.info(`getActiveTemplate called with clinicId=${clinicId}, type=${type}`);
    // First try to get clinic's custom template
    const customTemplate = await index_js_1.db.query.emailTemplates.findFirst({
        where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.clinicId, clinicId), (0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.type, type), (0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.isActive, true)),
    });
    if (customTemplate) {
        logger_js_1.logger.info(`Found custom template: ${customTemplate.name} (id: ${customTemplate.id})`);
        return customTemplate;
    }
    logger_js_1.logger.info(`No custom template found, using default for type: ${type}`);
 ...
 */
const createTemplate = async (clinicId, data) => {
    const [template] = await index_js_1.db
        .insert(schema_js_1.emailTemplates)
        .values({
        clinicId,
        type: data.type,
        name: data.name,
 ...
 * Update a template
 */
const updateTemplate = async (id, clinicId, data) => {
    // If activating this template, deactivate others of the same type
    if (data.isActive === true) {
        // First get the template to know its type
        const template = await index_js_1.db.query.emailTemplates.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.id, id), (0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.clinicId, clinicId)),
        });
        if (template) {
            // Deactivate all other templates of the same type (excluding current)
            await index_js_1.db
                .update(schema_js_1.emailTemplates)
                .set({ isActive: false, updatedAt: new Date() })
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.clinicId, clinicId), (0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.type, template.type), (0, drizzle_orm_1.ne)(schema_js_1.emailTemplates.id, id) // Exclude current template
            ));
        }
    }
 ...
 * Delete a template
 */
const deleteTemplate = async (id, clinicId) => {
    await index_js_1.db
        .delete(schema_js_1.emailTemplates)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.id, id), (0, drizzle_orm_1.eq)(schema_js_1.emailTemplates.clinicId, clinicId)));
    return true;
};
    ...
    const blockHtmls = blocks.map((block) => {
        ...
        switch (block.type) {
            ...
            case 'button': {
                ...
                let buttonText, buttonUrl, buttonColor;
                if (typeof block.content === 'object' && block.content !== null) {
