...
### Docker & Servicios
- [ x] `docker compose up -d` levanta todos los contenedores sin errores
...
- [ x] `npm run central:push` crea las tablas (`tenants`, `global_users`, `superadmins`)
- [x ] `npm run central:studio` abre Drizzle Studio correctamente
...
### Provisioning
- [x ] Crear SUPERADMIN con `--create-superadmin`
- [ x] Provisionar Tenant A (ej: `mi-clinica`)
...
- [ x] "Olvidé mi contraseña" envía email con enlace
- [x ] Enlace de reset funciona (token válido)
- [ x] Token expirado muestra mensaje de error
- [ x] Usuario en 1 tenant → formulario directo
- [ x] Usuario en 2+ tenants → selector de empresa
- [ x] Cambiar contraseña en empresa seleccionada
- [ x] Tokens se limpian de las demás empresas
- [ x] No se revela si el email existe o no (seguridad)
...
### Logout
- [ x] Cerrar sesión invalida el token
- [ x] Refresh token deja de funcionar tras logout
...
### Organización
- [ x] Ver datos de la organización
- [ x] Editar nombre, email, teléfono, dirección

### Clínicas (CRUD)
- [x ] Crear nueva clínica
- [x] Editar clínica existente
- [x ] Ver lista de clínicas
- [x] Los horarios se configuran correctamente (08:00-22:00)
- [x] Cambiar de clínica (context switching) funciona
- [x] Al cambiar de clínica se recarga la página correctamente
...
### CRUD de Personal
- [ x] Crear nuevo trabajador (WORKER)
- [ x] Crear nuevo admin (ADMIN)
- [ x] Editar datos del trabajador
- [ x] Desactivar un trabajador
